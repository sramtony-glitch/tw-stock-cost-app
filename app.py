...
# 1. 網頁頁面基本設定
st.set_page_config(page_title="籌碼成本線分析 App (FinMind 100%真實版)", layout="wide", initial_sidebar_state="expanded")
...
st.title("📈 散戶 vs 法人籌碼成本分析 App (100% FinMind 真實數據)")

# FinMind API Token 設定
FINMIND_TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjoic3JhbXRvbnlAZ21haWwuY29tIiw11haWwOiJzcmFtdG9ueUBnbWFpbC5jb20iLCJ0b2tlbl92ZXJzaW9uIjo18LWvBBvbrpnVzWKlOu2ZNIkshuwTnLKRQpBmcMPQ"
...
# 自 FinMind 官方資料庫抓取 100% 逐日真實籌碼數據
...
# 規格書 7 步驟洗淨演算法
def process_spec_chip_algorithm_finmind(df_price, df_chip):
    ...
    # Step 4: 真實融資斷頭日偵測與重置 (margin_delta / margin_balance[t-1] <= -3%)
...
if stock_code:
    try:
        with st.spinner("正在連線 FinMind 資料庫獲利 100% 真實籌碼數據..."):
        ...
        if df_price.empty:
            ...
            df = process_spec_chip_algorithm_finmind(df_price, df_chip)

            st.markdown(f"## 📌 **{stock_name} ({stock_code})** - 100% 真實籌碼戰術地圖")
